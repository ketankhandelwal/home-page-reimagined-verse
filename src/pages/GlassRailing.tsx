
import Header from "@/components/Header";
import Footer from "@/components/Footer";

const GlassRailing = () => {
  return (
    <div className="min-h-screen bg-white">
      <Header />
      
      {/* Hero Section */}
      <div className="relative h-96 bg-gradient-to-r from-yellow-600 to-yellow-700">
        <div className="absolute inset-0 bg-black bg-opacity-40"></div>
        <div className="relative container mx-auto px-4 h-full flex items-center">
          <div className="text-white animate-fade-in">
            <h1 className="text-5xl font-bold mb-4">Glass Railing</h1>
            <p className="text-xl">Modern and elegant glass railing solutions</p>
          </div>
        </div>
      </div>

      {/* Products Grid */}
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:-translate-y-2 animate-fade-in" style={{animationDelay: '0.1s'}}>
            <div className="h-48 bg-gradient-to-br from-yellow-100 to-yellow-200 flex items-center justify-center transition-all duration-300 hover:from-yellow-200 hover:to-yellow-300">
              <span className="text-yellow-600 font-semibold text-lg">Balcony Glass Railing</span>
            </div>
            <div className="p-4">
              <h3 className="font-bold text-lg mb-2 text-gray-800">Balcony Glass Railing</h3>
              <p className="text-gray-600 text-sm">Safe and stylish glass railings for balconies</p>
            </div>
          </div>

          <div className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:-translate-y-2 animate-fade-in" style={{animationDelay: '0.2s'}}>
            <div className="h-48 bg-gradient-to-br from-yellow-100 to-yellow-200 flex items-center justify-center transition-all duration-300 hover:from-yellow-200 hover:to-yellow-300">
              <span className="text-yellow-600 font-semibold text-lg">Staircase Glass Railing</span>
            </div>
            <div className="p-4">
              <h3 className="font-bold text-lg mb-2 text-gray-800">Staircase Glass Railing</h3>
              <p className="text-gray-600 text-sm">Premium glass railings for staircases</p>
            </div>
          </div>

          <div className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:-translate-y-2 animate-fade-in" style={{animationDelay: '0.3s'}}>
            <div className="h-48 bg-gradient-to-br from-yellow-100 to-yellow-200 flex items-center justify-center transition-all duration-300 hover:from-yellow-200 hover:to-yellow-300">
              <span className="text-yellow-600 font-semibold text-lg">Terrace Glass Railing</span>
            </div>
            <div className="p-4">
              <h3 className="font-bold text-lg mb-2 text-gray-800">Terrace Glass Railing</h3>
              <p className="text-gray-600 text-sm">Weather resistant glass railings for terraces</p>
            </div>
          </div>
        </div>
      </div>

      <Footer />
    </div>
  );
};

export default GlassRailing;
