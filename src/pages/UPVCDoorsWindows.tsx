  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-gray-50">
        ...
        <div className="relative container mx-auto px-4 h-full flex items-center justify-center">
          <div className="text-white text-center animate-fade-in">
            <div className="inline-block px-6 py-2 bg-white/10 backdrop-blur-md rounded-full mb-6 border border-white/20">
              <span className="text-yellow-300 font-semibold text-sm uppercase tracking-wider">Energy Efficient</span>
            </div>
            ...
            <p className="text-2xl text-gray-200 font-light">Energy efficient UPVC doors and windows</p>
      ...
      <div className="container mx-auto px-4 py-20 relative">
        <div className="absolute top-10 right-10 w-72 h-72 bg-gradient-to-br from-green-200/20 to-blue-200/20 rounded-full blur-3xl"></div>
        ...
        <div className="text-center mb-16 animate-fade-in relative z-10">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Durable, energy-efficient UPVC doors and windows for modern homes and offices
          </p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
          {[
            { title: 'UPVC Windows', desc: 'Energy efficient and weather resistant windows' },
            { title: 'UPVC Doors', desc: 'Durable and low maintenance UPVC doors' },
            { title: 'Sliding Windows', desc: 'Space-saving sliding UPVC windows' }
          ].map((product, index) => (
            <div key={index} className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden transform transition-all duration-700 hover:scale-105 hover:-translate-y-4 animate-fade-in border border-gray-100/50" style={{animationDelay: `${index * 0.1}s`}}>
              <div className="h-56 bg-gradient-to-br from-green-100 to-blue-100 flex items-center justify-center transition-all duration-500 group-hover:from-green-200 group-hover:to-blue-200 relative overflow-hidden">
                ...
                <span className="text-green-700 font-bold text-2xl transform group-hover:scale-110 transition-transform duration-500 relative z-10">{product.title}</span>
                <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
              </div>
              <div className="p-6 bg-gradient-to-br from-white to-gray-50/50">
                <h3 className="font-bold text-xl mb-3 text-gray-800 group-hover:text-green-600 transition-colors duration-300">{product.title}</h3>
                <p className="text-gray-600 leading-relaxed">{product.desc}</p>
                <div className="mt-4 w-12 h-1 bg-gradient-to-r from-green-400 to-blue-400 rounded-full transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>
              </div>
            </div>
          ))}
        </div>
